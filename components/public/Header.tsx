  ...
  return (
    <ScrollContext.Provider value={scrolled}>
      <div className="fixed top-0 left-0 z-50 w-full">
        ...
        <div
          className={cn(
            "w-full bg-black max-md:hidden border-b border-white/10 overflow-hidden transition-all duration-300",
        ...
        {/* ─── MAIN NAVIGATION ─── */}
        <div className="w-full border-b border-white/10 bg-black backdrop-blur-xl px-5">
  ...
  <nav className="hidden lg:flex items-center gap-6 xl:gap-10 mx-auto">
    {siteConfig.navItems.map((item) => (
      <div key={item.label} className="relative group">
        {item.items ? (
          <button className="flex items-center gap-1 font-bold uppercase tracking-[1.5px] text-white/80 hover:text-white text-[12px] transition-colors">
            {item.label}
            <ChevronDown
              size={14}
          ...
          <Link
            href={item.href}
            className="font-bold uppercase tracking-[1.5px] text-white/80 hover:text-white text-[12px] transition-colors"
          >
            {item.label}
          </Link>
        )}
        ...
        {item.items && (
          <div className="absolute top-full left-0 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
            <div className="bg-black border border-white/20 rounded-lg p-2 min-w-[220px] shadow-2xl flex flex-col">
